...
class malacc(commands.Cog):
    ...
    async def mal(self, ctx, message=""):
        ...
        async with ctx.channel.typing():
            ...
            data = {
                ...
                "img": (
                    "https://i.imgur.com/9mnjji8.png"
                    if (user.get("image_url") == None)
                    else user.get("image_url")
                ),
            ...
            embed = discord.Embed(
                title="**" + data["username"] + "**", colour=0x000CFF, url=data["url"]
            )
    ...
    async def malset(self, ctx, message=""):
        ...
        if [(1,)] == data:
            cur.execute(
                "UPDATE mal SET username = ? WHERE userid = ?",
                (message, str(ctx.author.id)),
            )

        else:
            cur.execute(
                "INSERT INTO mal (userid, username) VALUES (?,?)",
