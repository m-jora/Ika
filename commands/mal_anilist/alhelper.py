...
class alhelper(commands.Cog):
    ...
    async def isaccount(self, username):
        variables = {"search": username, "page": 1}
  ...
  send an embed with instructions on how to verify"""
    ...
    async def validate(self, ctx, username):
        ...
        cur.execute(
            "SELECT EXISTS (SELECT userid FROM al WHERE userid = ?)",
            (str(ctx.author.id),),
        )
        data = cur.fetchall()
        ...
        if data == [(1,)]:
            cur.execute(
                "SELECT username FROM al WHERE userid = ?", (str(ctx.author.id),)
            )
            data = cur.fetchall()[0][0]
            conn.close()

            await ctx.send(
                f"You have already verified the account {data}.\nUse m:alremove if you wish to remove this account."
            )
            return

        embed = discord.Embed(title="**Account Verification**", colour=0x000CFF)
        ...
        title = (
            ...
            if done[0]
            else "**Failed to find the key in your description within time limit**"
        )
        url = (
            "https://i.imgur.com/DkuetBb.gif"
            if done[0]
            else "https://i.imgur.com/qSCvqai.gif"
        )
        ret_val = [True, done[1]] if done[0] else [False, ""]

        embed = discord.Embed(title=title, colour=0x000CFF)
