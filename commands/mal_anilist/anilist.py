  ...
  sends embed of the anilist image for the account"""
    ...
    async def anilist(self, ctx, message=""):
        ...
        if message[:2] == "<@" and message[-1] == ">":
            message = await self.alaccount(ctx, message[3:-1])
            db = True
            if message is "":
                await ctx.send("User has not linked an anilist account")
                db = False
                return
    ...
    """
        ...
        embed.set_image(
            url=f'https://img.anili.st/user/{str(user[0]["id"])}?{str(random.randint(0, 9999999))}'
        )
  ...
  """
    ...
    async def alset(self, ctx, message=""):
        ...
        if connect[0] is True:
            cur.execute(
                "INSERT INTO al (userid, username, alid) VALUES (?,?,?)",
                (str(ctx.author.id), message, str(connect[1])),
            )
            conn.commit()
            conn.close()
