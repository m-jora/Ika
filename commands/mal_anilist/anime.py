...
class anime(commands.Cog):
    ...
    async def malani(self, ctx, message):
        ...
        async with ctx.channel.typing():
            ...
            info["start"] = (
                "Not Yet Aired" if "None" in info["start"] else info["start"]
            )
            info["end"] = (
                "-"
                if info["start"] is "Not Yet Aired" and "None" in info["end"]
                else (
                    "Currently Airing"
                    if "Not Yet Aired" != info["start"] and "None" in info["end"]
                    else info["end"]
                )
            )
  ...
  """
            ...
            variables = {"search": message, "page": 1, "type": "ANIME"}
            ...
            info[
                "start"
            ] = f'{media["startDate"]["month"]}-{media["startDate"]["day"]}-{media["startDate"]["year"]}'
            info[
                "end"
            ] = f'{media["endDate"]["month"]}-{media["endDate"]["day"]}-{media["endDate"]["year"]}'
            ...
            info["end"] = (
                "-"
                if info["start"] is "Not Yet Aired"
                else (
                    "Currently Airing"
                    if "Not Yet Aired" != info["start"] and "None" in info["end"]
                    else info["end"]
            ...
            embed.add_field(
                name="Score / Popularity / Rank",
                value=f'Score: {info["score"]} / Popularity: {str(info["popularity"])} / Rank: {str(info["rank"])}',
                inline=False,
            )
            embed.add_field(name="Started Airing", value=info["start"])
            embed.add_field(name="Finished Airing", value=info["end"])
