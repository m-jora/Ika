...
class info(commands.Cog):
    ...
    @cog_ext.cog_slash(
        name="botstatus", description="Bot Info", guild_ids=[840366699353866271]
    )
    async def botstatus(self, ctx):
        embed = discord.Embed(title="**Bot Status**", colour=0x09D2FF)
        ...
        embed.add_field(
            name=f"**{mem.display_name} is in:**", value=f"{str(len(servers))} servers"
        )
        embed.add_field(
            name=f"**{mem.display_name} is watching:**", value=f"{str(members)} users"
        )
        embed.add_field(
            name=f"**{mem.display_name} is watching:**",
            value=f"{str(channel)} channels",
        )
    ...
    @cog_ext.cog_slash(
        name="Server", description="Server Stats", guild_ids=[840366699353866271]
    )
    async def server(self, ctx):
        embed = discord.Embed(title=f"**{ctx.guild.name}**", colour=0x09D2FF)

        embed.set_thumbnail(url=ctx.guild.icon_url)
        embed.add_field(
            name="**Server Created:**", value=str(ctx.guild.created_at)[:10]
        )
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            create_option(name="user", description="pain", required=True, option_type=6)
        ],
    )
    async def userinfo(self, ctx, user):
        embed = discord.Embed(title="**User Info**", colour=0x09D2FF)
